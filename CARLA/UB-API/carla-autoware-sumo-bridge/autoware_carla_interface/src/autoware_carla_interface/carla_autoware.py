...
class SensorLoop(object):
    ...
    def _tick_sensor(self, timestamp):
        ...
        # Advance CARLA only when this loop is the designated time master.
        if self.running and not self.external_tick:
    ...
    def run_bridge(self):
        ...
        while self.bridge_loop.running:
            ...
            if timestamp:
                ...
                # Only throttle to real-time when this process is the time master.
                if (not self.external_tick) and (delta_step <= self.max_real_delta_seconds):
